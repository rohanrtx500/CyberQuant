...
import { Target, CheckCircle2 } from "@/components/icons";
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white dark:bg-slate-900 p-4 rounded-lg border border-slate-200/90 dark:border-slate-800 shadow-xs transition-colors">
          <div>
            <div className="flex items-center space-x-2">
              <h1 className="text-lg font-bold tracking-tight text-slate-900 dark:text-slate-100">Cybersecurity Investment Optimizer</h1>
          ...
          <div className="text-xs text-slate-600 dark:text-slate-300 bg-slate-50 dark:bg-slate-800 px-3 py-1.5 rounded border border-slate-200 dark:border-slate-700 font-mono">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex-1">
              <div className="flex items-center justify-between mb-1.5">
                ...
                <span className="text-base font-bold font-mono text-blue-600 dark:text-blue-400">
                  {formatINR(budget)} <span className="text-xs font-normal text-slate-500 dark:text-slate-400">({formatINRFull(budget)})</span>
                </span>
              </div>
            ...
            <div className="flex items-center space-x-2">
              <button
                ...
                disabled={loading}
                className="bg-blue-600 hover:bg-blue-700 text-white font-semibold text-xs px-4 py-2.5 rounded-lg shadow-xs transition flex items-center space-x-2 disabled:opacity-50 cursor-pointer"
              >
          ...
          <>
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-lg shadow-xs overflow-hidden transition-colors">
              <div className="p-3.5 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
                <div>
                  <h2 className="text-xs font-bold text-slate-800 dark:text-slate-100 tracking-tight flex items-center space-x-1.5">
                ...
                <span className="text-[11px] font-semibold text-emerald-700 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950/40 px-2.5 py-1 rounded border border-emerald-200 dark:border-emerald-800">
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                    {optResult.selected_initiatives.map((item) => (
                      <tr key={item.id} className="hover:bg-slate-50/70 dark:hover:bg-slate-800/50 transition-colors">
                        ...
                        <td className="py-2.5 px-3 text-right font-mono tabular-nums text-slate-900 dark:text-slate-100 font-bold">
                          {formatINR(item.cost)}
                        </td>
            ...
            {optResult.unselected_initiatives.length > 0 && (
              <div className="bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-lg shadow-xs overflow-hidden opacity-90 transition-colors">
                <div className="p-3.5 border-b border-slate-200 dark:border-slate-800">
                  <h2 className="text-xs font-bold text-slate-700 dark:text-slate-300 tracking-tight">Deferred Initiatives (Exceeds Current Budget)</h2>
                  <p className="text-[11px] text-slate-500 dark:text-slate-400">Candidate controls deferred due to lower marginal return per rupee</p>
                </div>
                  ...
                  <table className="w-full text-left text-xs">
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                      {optResult.unselected_initiatives.map((item) => (
                        <tr key={item.id} className="text-slate-500 dark:text-slate-400 hover:bg-slate-50/70 dark:hover:bg-slate-800/50">
                          <td className="py-2 px-3.5 font-medium text-slate-800 dark:text-slate-200">{item.name}</td>
